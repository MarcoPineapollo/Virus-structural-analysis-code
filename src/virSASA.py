...
def resSASA_extract(structure, resname=None, chainid=None):
    ...
    for chain in model:
        raw_chain_id = chain.get_id()
        
        # re.split(r'[-_]', text) splits on EITHER a hyphen or an underscore plus the chain number
        # Then we take [0] to grab the core chain ID
        clean_chain = re.split(r'[-_]', raw_chain_id)[0]
            
        for res in chain:
            complete_list.append((clean_chain, res.get_id()[1], res.get_resname(), round(res.sasa, 2)))

    df = pd.DataFrame(complete_list, columns=['chainid', 'resnum', 'resname', 'SASA'])
    
    # Sums together the SASA values for the unified chain IDs
